...
<script type="text/javascript">
	...
	function passImage(){
		var row = $('#dg').datagrid('getSelected');
	    if (row){
	    	$.messager.confirm('审核确认','你确定要通过栏目‘'+row.title+'’审核吗？',function(r){
	            if (r){
	                $.post('<%=path%>/AdminServlet?method=passImage',{"id":row.id},function(result){
	                    if (result[0].success){
	                    	$('#dg').datagrid('options').url = "<%=path%>/AdminServlet?method=showImages" ;
...
<!-- 	datagrid -->
	<table id="dg" class="easyui-datagrid" style="" toolbar="#toolbar"
			url="<%=path%>/AdminServlet?method=showImages"
			data-options="
				lines: true,
				idField: 'id',
				rownumbers:true,
...
<!-- 	操作对话框 -->
	...
	<div id="toolbar" class="datagrid-toolbar">		
		<a href="#" class="easyui-linkbutton" data-options="iconCls:'icon-add'"plain="true" onclick="queryImage()" >查看详细</a>  <!-- onclick="queryImage()" -->
		<a href="#" class="easyui-linkbutton" data-options="iconCls:'icon-edit'" plain="true"onclick="passImage()">审核通过</a>
	...
	<div id="mm" class="easyui-menu" data-options="hideOnUnhover:false"style="width:120px;">
		...
		<div data-options="iconCls:'icon-edit'" onclick="passImage()">审核通过</div>
